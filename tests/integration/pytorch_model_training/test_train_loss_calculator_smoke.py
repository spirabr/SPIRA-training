...
def test_train_loss_calculator_smoke(train_loss_calculator: PytorchLossCalculator):
    try:
        train_loss_calculator.calculate_loss(
            predictions=torch.tensor([1.0]), labels=torch.tensor([1.0])
        )
        train_loss_calculator.recalculate_weights()
